...
let PouchBase = function (host, PouchDB) {
  // In-memory store for tokens so the tests can fetch the unencrypted token without going via email.
  this.cachedTokens = {};
...
PouchBase.prototype.sendToken = function (email, token, host, cb) {
  ...
  if (!process.env.EMAIL_ADDRESS ||
    ...
    cb();
    console.log('Not sending email, no credentials');
...
PouchBase.prototype.tokenUrl = function (token, host, uid) {
  return this.host + 'validate/?token=' + token +
    '&uid=' + encodeURIComponent(uid) +
    '&host=' + encodeURIComponent(host);
};

PouchBase.prototype.login = function (details, origin) {
...
PouchBase.prototype.authenticate = function (user, origin, token) {
          ...
          db.get(activeUserId(user, origin), function (err, doc) {
            ...
            db.put(doc).then(function () {
              db.remove(tokenDoc).then(function () {
                console.log('Authenticating ' + user + ' was successful');
                resolve({
                  ok: true,
                  origin: origin
                });
...
PouchBase.prototype.readSession = function (user, host) {
  return new Promise(function (resolve) {
...
PouchBase.prototype.writeSession = function (user, host, details) {
  return new Promise(function (resolve) {
    this.db.get(activeUserId(user, host), function (err, doc) {
...
PouchBase.prototype.usersDbName = function (user, host) {
  return encodeURIComponent(user) + '_' + encodeURIComponent(host);
};

// Exposed for testing.
PouchBase.prototype.fetchTokenUrl = function (user, host) {
